...
qrencryptor is a free  web app that provides secure file encryption in the browser.
...
---
It's easy to self host and deploy qrencryptor, you can do that either with npm or docker
...
```bash
git clone https://github.com/fightagainstdev
```
...
```bash
docker build . -t qrencryptor
```

4. run container

```bash
docker run -d -p 3991:80 qrencryptor
```
...
qrencryptor is also available as a Docker image. You can find it on [Docker Hub].
...
Files encrypted with qrencryptor are identifiable by looking at the file signature that is used by the app to verify the content of a file, Such signatures are also known as magic numbers or Magic Bytes. These Bytes are authenticated and cannot be changed.
...
Yes, Hat.sh is free and always will be. However, please consider [donating](https://fightagainstdev.github.io/donate/) to support the project.
...
qrencryptor is an open-source application. You can help make it better by making commits on GitHub. The project is maintained in my free time. [Donations](https://github.com/fightagainstdev) of any size are appreciated.
